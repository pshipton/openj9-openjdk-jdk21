                             ...
                             jobject fdo, jobject iao, jint port)
{
    ...
    if (AF_INET == sa.sa4.sin_family) {
        ...
        Trc_nio_ch_Net_connect4(fd, inet_ntop(AF_INET, &sa.sa4.sin_addr, buf, sizeof(buf)), port, sa_len);
    } else if (AF_INET6 == sa.sa6.sin6_family) {
        char buf[INET6_ADDRSTRLEN];
        Trc_nio_ch_Net_connect6(fd, inet_ntop(AF_INET6, &sa.sa6.sin6_addr, buf, sizeof(buf)), port, ntohs(sa.sa6.sin6_scope_id), sa_len);
    }
